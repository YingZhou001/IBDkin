...
void p_parameters(void)
{
    ...
    fprintf(stdout, " ibd: *%s*\n", headfile);
    read_headfile(headfile);
    fprintf(stdout, " map: *%s*\n", mapfile);
    fprintf(stdout, " ind: *%s*\n", idfile);
    ...
    if(tagMask == 1)fprintf(stdout, "  *%s*\n", maskfile);
    if(tagCoverage == 1)fprintf(stdout, "  *%s*\n", coveragefile);

    fprintf(stdout, "\nOther settings:\n");
    fprintf(stdout, " *ibd cutoff-1 = %fcM*\n", IBDcM);
    fprintf(stdout, " *ibd cutoff-2 = %fcM*\n", IBDcM2);
    fprintf(stdout, " *degree = %d, kinship = %f*\n",degree, kincut);
    fprintf(stdout, " *fold = %f*\n", FOLD);
    fprintf(stdout, " *binsize = %.3fkb*\n", BINSIZE);
    ...
    if(gap1 > 0){
	fprintf(stdout, " *merge threshold for IBD1 = %fcM*\n", gap1);
	fprintf(stdout, " *merge threshold for IBD2 = %fcM*\n", gap2);
    }
    else fprintf(stdout, " **no merge**\n");
    if(Parts > 1)fprintf(stderr, " **%d partitions in total, this is the %d's run**\n", Parts, part);
    else fprintf(stdout, " **no partitions**\n");
...
void p_mem(void)
{
    ...
    if(mem > 1){fprintf(stdout, " *Memory in use: %.5lf Gigabytes\n\n", mem);return;}
    mem = (double)tmp / 1048576;
    if(mem > 1){fprintf(stdout, " *Memory in use: %.5lf Megabytes\n\n", mem);return;}
    mem = (double)tmp / 1024;
    if(mem > 1){fprintf(stdout, " *Memory in use: %.5lf Kilobytes\n\n", mem);return;}
...
void p_maxrss(void)
{
    ...
    mem = (double)tmp / 1048576;
    if(mem > 1){fprintf(stdout, " *maxrss: %.5lf Gigabytes\n", mem);return;}
    mem = (double)tmp / 1024;
    if(mem > 1){fprintf(stdout, " *maxrss: %.5lf Megabytes\n", mem);return;}
    fprintf(stdout, " *maxrss: %.5lf Kilobytes\n", tmp);
    return ;
}
