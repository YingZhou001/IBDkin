...
{
    fprintf(stdout, "############################\n");
    fprintf(stdout, "#Welcome to IBDkin V2.8.7.4#\n");
    fprintf(stdout, "############################\n");
    fprintf(stdout, "Copyright (C) 2020 Ying Zhou\n\n");
...
void read_ibd_1st_pass(void)
{
	...
	j = 0;
        while(j < buffi2){
            if(sscanf(strbuff2[j], "%*s %*s %*s %*s %d", &CHR)!=1)j++;
            else break;
        }
        if(j == buffi2){
            fprintf(stderr, "Error, wrong chromosome name!\n");
            exit(-1);
        }
...
//fprintf(stderr, "store_buff_ibd()\n");
	...
	if( minPos[i] < maxPos[i]){
	    ...
	    fprintf(stdout, "\tchr%d: %.2f -> %.2f cM, masked length = %.2fcM\n",i, minPos[i], maxPos[i], maskL);
	}

    }
    fprintf(stdout, "  Total chromosome length: %f cM\n", totg);
    if(checkTime ==1 ){
...
void read_ibd_2nd_pass(void)
{
    ...
    if(checkTime){
	p_std_time(my_wallclock() - a);
	fprintf(stdout,"read_ibd_2nd_pass() Time = %s\n", timestr);
    }
}
...
void read_map(char *plinkmap)
{
    ...
    //while(gzgets(str, BUFF-1, ifp)!=NULL){
    while(gzgets(fp, str, BUFF-1)!=NULL){
	chr = -1;
	if(sscanf(str, "%d", &chr) == 1){
	    if(chr > 0 && chr < 23)mapL[chr] ++;
	}
    }
    gzclose(fp);
    ...
    while(gzgets(fp, str, BUFF-1)!=NULL){
	if(sscanf(str, "%d %*s %f %f", &chr, &g, &p)==3){
	    if(chr > 0 && chr < 23){
		mapG[chr][mapi[chr]] = g;
		mapP[chr][mapi[chr]] = p;
		mapi[chr] ++;
    ...
    p_std_time(my_wallclock() - a);
    fprintf(stdout,"read_genetic_map() Time = %s\n", timestr);
    return ;
}
...
void read_ind(char *idfile)
{
    ...
    fprintf(stdout, "%d individuals\n", idNum);
    p_std_time(my_wallclock() - a);
    fprintf(stdout,"read_ind() Time = %s\n", timestr);
    return ;
}
...
void init(void)
{
    ...
    fprintf(stdout, "allocate IBD buffer\n");
